...
#if defined(NDEBUG)
# error "Threeitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x59ffe119282b34f0670d421b82b60ec74b2c383444e1b8b0b12ab037934c1cdf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 1); // Threeitcoin: starting difficulty is 1 / 2^26
...
const string strMessageMagic = "Threeitcoin Signed Message:\n";
...
{
    // Threeitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+6) - GetDepthInMainChain());
}
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut to zero after 1 block, which will occur in premine, no block reward after block 1
    nSubsidy >>= (nHeight / 21000000000); // Threeitcoin: 21 billion blocks in alltime

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 15; // Threeitcoin: 15 seconds
static const int64 nTargetSpacing = 1 * 1; // Threeitcoin: 1 second
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Threeitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Threeitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x59ffe119282b34f0670d421b82b60ec74b2c383444e1b8b0b12ab037934c1cdf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC UK NEWS, 17 April 2018 Berkshire, Extra trains for royal wedding visitors";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043989710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b04b6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523961779;
        ...
        assert(block.hashMerkleRoot == uint256("0x2c41a60a82ce8f698cc6d64294802a93b6c64c294af3111e6846fb73db32f19b"));
...
unsigned char pchMessageStart[4] = { 0xd3, 0xb4, 0xa2, 0xbd }; // Threeitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ThreeitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ThreeitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreeitcoinMiner(CWallet *pwallet)
{
    printf("ThreeitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("threeitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ThreeitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ThreeitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreeitcoinMiner, pwallet));
}
